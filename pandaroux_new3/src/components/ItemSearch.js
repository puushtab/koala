...
const ItemSearch = ({ category, onBack, onItemClick }) => {
  ...
  return (
    <div className="item-search">
      ...
      <div className="dev-note slide-up" style={{ animationDelay: '300ms' }}>
        <div className="dev-note-icon">💡</div>
        <div className="dev-note-content">
          <p className="dev-note-title">Zone d'intégration LLM</p>
          <p className="dev-note-text">
            La fonction <code>handleSearch()</code> est prête pour l'intégration du backend LLM.
            Les résultats seront affichés automatiquement via le composant ResultCard.
          </p>
          <p className="dev-note-text" style={{ marginTop: '0.5rem' }}>
            Format attendu : array d'objets avec <code>id</code>, <code>title</code>, <code>price</code>, 
            <code>photo</code>, <code>url</code>, <code>brand</code>, <code>size</code>, 
            <code>price_difference</code> (optionnel), <code>carbon_impact_kg</code> (optionnel)
          </p>
        </div>
      </div>
    </div>
